...
static size_t _out_rev(out_fct_type out, char* buffer, size_t idx,
                       size_t maxlen, const char* buf, size_t len, size_t width,
                       size_t flags) {
...
static size_t _ntoa_format(out_fct_type out, char* buffer, size_t idx,
                           ...
                           size_t base, size_t prec, size_t width,
                           size_t flags) {
    // pad leading zeros
    if (!(flags & FLAGS_LEFT)) {
...
static size_t _ntoa_long_long(out_fct_type out, char* buffer, size_t idx,
                              ...
                              size_t prec, size_t width, size_t flags) {
    char buf[PRINTF_NTOA_BUFFER_SIZE] = {0};
    size_t len = 0U;
...
static int _vsnprintf(out_fct_type out, char* buffer, const size_t maxlen,
    ...
    while (*format) {
        // format specifier? %[flags][width][.percision][length]
        if (*format != '%') {
            // no
        ...
        if (*format == '.') {
            ...
            if (_is_digit(*format)) {
                ...
                precision = prec > 0 ? (size_t)prec : 0U;
                format++;
            }
        }
        ...
        switch (*format) {
            ...
            case 'b': {
                ...
                if (*format == 'x' || *format == 'X') {
                    ...
                    base = 10U;
                    flags &= ~FLAGS_HASH; // no has for dec format
                }
                // uppercase
                if (*format == 'X') {
                ...
                if ((*format == 'i') || (*format == 'd')) {
                    ...
                    // unsigned
                    // TODO: code duplication with "convert to integer"
                    if (flags & FLAGS_LONG_LONG) {
    ...
    // return writen chars without termination \0
    return (int)idx;
}
